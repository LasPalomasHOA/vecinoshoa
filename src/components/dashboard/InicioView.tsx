  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="relative rounded-3xl overflow-hidden border border-slate-200/90 shadow-md no-print print:hidden">
        ...
        {/* Subtle, crystal directional tint to ensure high contrast without washing out the photo */}
        <div className="absolute inset-0 bg-gradient-to-r from-white/90 via-white/45 to-transparent pointer-events-none" />
        <div className="absolute inset-0 bg-gradient-to-t from-white/30 via-transparent to-black/10 pointer-events-none" />
        ...
        <div className="relative z-10 p-5 sm:p-6 lg:p-7 flex flex-col justify-between gap-6 min-h-[220px]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1.5 text-xs font-black px-3 py-1 rounded-full bg-emerald-600/95 backdrop-blur-md text-white shadow-xs">
                <span className="w-2 h-2 rounded-full bg-emerald-200 animate-pulse" />
                Operaciones HOA 2026
              </span>
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-800 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full border border-white/80 shadow-xs capitalize hidden sm:inline-flex">
                <Calendar className="w-3.5 h-3.5 text-slate-500" /> {todayFormatted}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="h-9 px-3.5 rounded-xl bg-white/95 hover:bg-white text-slate-900 border border-white/90 font-extrabold text-xs shadow-md backdrop-blur-md flex items-center gap-1.5 transition-all cursor-pointer active:scale-95 hover:border-teal-400"
              >
                <KeyRound className="w-3.5 h-3.5 text-teal-700 stroke-[2.5]" />
                <span>Front Desk</span>
              </button>

              <button
                onClick={onOpenNewReservation}
                className="h-9 px-4 rounded-xl bg-teal-700 hover:bg-teal-800 text-white font-black text-xs shadow-md backdrop-blur-md flex items-center gap-1.5 transition-all cursor-pointer active:scale-95"
              >
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
            ...
            <div className="space-y-2 max-w-2xl">
              <div className="space-y-1">
                <h1 className="text-2xl sm:text-3xl lg:text-[32px] font-black tracking-tight text-slate-950 leading-tight">
                  Las Palomas Seaside Golf Community
                </h1>
                {currentUser && (
                  <div className="pt-0.5">
                    <span className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-extrabold px-3 py-1 rounded-xl bg-white/90 backdrop-blur-md text-teal-950 border border-teal-200/80 shadow-xs">
                      <Sparkles className="w-3.5 h-3.5 text-teal-700" />
                      <span>Bienvenido, <strong className="font-black text-teal-950">{currentUser.nombre} {currentUser.apellido}</strong></span>
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2 p-1.5 sm:p-2 rounded-2xl bg-white/85 backdrop-blur-xl border border-white/80 shadow-lg shrink-0">
              ...
              <div className="px-3.5 py-1.5 rounded-xl hover:bg-white/60 transition-colors">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] uppercase font-black tracking-wider text-slate-500">En Casa</span>
                </div>
                <div className="mt-0.5 flex items-baseline gap-1">
                  <span className="text-xl font-black text-slate-950">{inHouseReservations.length}</span>
                  <span className="text-[11px] text-emerald-800 font-extrabold">activas</span>
                </div>
              </div>

              <div className="h-8 w-px bg-slate-200/80 my-auto" />

              {/* Llegadas */}
              <div className="px-3.5 py-1.5 rounded-xl hover:bg-white/60 transition-colors">
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-amber-500" />
                  <span className="text-[10px] uppercase font-black tracking-wider text-slate-500">Llegadas</span>
                </div>
                <div className="mt-0.5 flex items-baseline gap-1">
                  <span className="text-xl font-black text-amber-950">{pendingReservations.length}</span>
                  <span className="text-[11px] text-amber-800 font-extrabold">pendientes</span>
                </div>
              </div>

              <div className="h-8 w-px bg-slate-200/80 my-auto hidden sm:block" />

              {/* Condos */}
              <div className="px-3.5 py-1.5 rounded-xl hover:bg-white/60 transition-colors hidden sm:block">
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-teal-500" />
                  <span className="text-[10px] uppercase font-black tracking-wider text-slate-500">Condos</span>
                </div>
                <div className="mt-0.5 flex items-baseline gap-1">
                  <span className="text-xl font-black text-teal-950">{propiedades.length}</span>
                  <span className="text-[11px] text-teal-800 font-extrabold">Torres A-J</span>
                </div>
              </div>
      ...
      <div className="p-3 rounded-2xl bg-white border border-slate-200/90 shadow-2xs flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 bg-slate-100/90 p-1 rounded-xl border border-slate-200/80">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${selectedPeriod === 'current_month'
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${selectedPeriod === 'current_year'
              ? 'bg-white text-teal-900 shadow-xs border border-slate-200/60'
              : 'text-slate-700 hover:text-slate-950 font-bold'
              }`}
          >
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${selectedPeriod === 'all'
              ? 'bg-white text-teal-900 shadow-xs border border-slate-200/60'
              : 'text-slate-700 hover:text-slate-950 font-bold'
              }`}
          >
              ...
              towerOccupancy.map(t => (
                <div key={t.id} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                  ...
                  <div className="w-full bg-slate-200 h-3 rounded-full overflow-hidden border border-slate-300">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${t.percent > 75
                        ? 'bg-emerald-600'
                        : t.percent > 40
                          ? 'bg-teal-600'
                          : t.percent > 0
            ...
            return (
              <div key={m.monthIdx} className="flex flex-col items-center gap-1.5 group cursor-pointer">
                ...
                <div className="w-full max-w-[36px] bg-slate-100 h-28 rounded-lg overflow-hidden flex items-end border border-slate-200">
                  <div
                    className={`w-full rounded-lg transition-all duration-500 ${isCurrentMonth
                ...
                {/* Month Name */}
                <span className={`text-xs font-black uppercase ${isCurrentMonth ? 'text-teal-800 font-black underline' : 'text-slate-700'
                  }`}>
                  {m.name}
                </span>
              </div>
